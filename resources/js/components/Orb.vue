...
const props = withDefaults(
    defineProps<{
        ...
        size?: number;
        className?: string;
        /** Fill parent (e.g. hero column) */
        fill?: boolean;
        /** Kept for API compatibility; no effect on this orb */
        interactive?: boolean;
        /** Expanding ripple rings + stronger pulse (landing hero). */
        heroRipples?: boolean;
    }>(),
    {
        state: 'idle',
...
/** Softer ripples: opacity 0→peak→0 hides loop reset; eased multi-step scale avoids “popping”. */
...
<template>
    <div
        :class="cn('relative flex items-center justify-center', fill ? 'min-h-0 flex-1' : '', className)"
        :style="boxStyle"
    >
        ...
        >
            <motion.div
                ...
                :key="idx"
                class="absolute rounded-full border-2 border-brand/30 shadow-[0_0_24px_oklch(0.55_0.16_268_/_0.18)]"
